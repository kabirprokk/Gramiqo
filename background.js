...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === "GRAMI_GET_MEDIA" || msg.type === "INTA_GET_MEDIA") {
    // Only return full playable video files. Rank-0 URLs (DASH byte-range
    // segments) are just a slice of bytes and will produce an unsupported
    // file if downloaded directly. Skip them entirely.
    try {
      const arr = mediaByTab.get(sender?.tab?.id) || [];
      let best = null;
      for (const it of arr) {
        if (it.rank === 0) continue; // skip DASH segments
...
function guessExt(url) {
  try {
    ...
    if (clean.includes(".m4v")) return "m4v";
    if (clean.includes(".png")) return "png";
    if (clean.includes(".webp")) return "webp";
    // DASH segment URLs (bytestart/byteend) and dash video paths are video —
    // defaulting to .jpg here would save a video file as an image and make
    // it unplayable.
    if (clean.includes("bytestart") || clean.includes("byteend")) return "mp4";
    if (clean.includes("/dash/") || clean.includes("dash_") || clean.includes("video_dash")) return "mp4";
    if (clean.includes("mime=video") || clean.includes("mime=audio")) return "mp4";
    return "mp4";
  } catch {
    return "mp4";
  }
...
// Full progressive files (rank 2) beat extension-less video URLs
// (rank 1) beat range segments (rank 0, only a byte slice —
// undecodable on their own, must be skipped for direct download).
function rankMediaUrl(u) {
  try {
    ...
    if (s.includes("/dash/") || s.includes("dash_") || s.includes("video_dash")) return 1;
    return 1;
  } catch {
    return 0;
  }
