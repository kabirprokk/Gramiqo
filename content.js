...
/* ---------------- download (self-contained scraper) ---------------- */
...
function downloadUrl(url, filename) {
  ...
  if (url.startsWith("blob:")) {
    // Download blob directly via <a> — browser handles blob URLs natively.
    try {
      ...
      a.click();
      setTimeout(() => { if (a.parentNode) a.remove(); }, 30000);
      toast("Downloaded!");
    } catch {
      toast("Can't download this video (protected)");
    }
    return;
  }

  if (url.includes("bytestart") || url.includes("byteend")) {
    // DASH segment — need the full file. Scrape from Instagram page.
    toast("Finding full video…");
    scrapeInstagramVideoUrl().then((fullUrl) => {
      if (fullUrl) downloadUrl(fullUrl, filename);
      else toast("Can't get full video URL");
    });
    return;
  }

  // Main path: fetch the file directly and download via <a> blob.
  // This completely bypasses chrome.runtime.sendMessage and the
  // service worker — no dependency on background.js.
  fetchVideoFile(url, filename);
}

// Fetch the video file and trigger a native download.
async function fetchVideoFile(url, filename) {
  try {
    const resp = await fetch(url, { credentials: "include" });
    if (!resp.ok) return tryDownloadViaAnchor(url, filename);
    ...
    tryDownloadViaAnchor(url, filename);
  }
}

// Fallback: try downloading via <a> tag directly.
// Works for http/https URLs that the browser can handle.
...
// Scrape the current Instagram page to find the direct CDN video URL.
// Uses the page's own fetch to get page data, then parses for video sources.
async function scrapeInstagramVideoUrl() {
  try {
    // Method 1: Look for _sharedData or __additionalData in the page.
    // These are injected by Instagram's React hydration and contain
    // all media URLs including the highest quality video source.
    const scripts = document.querySelectorAll("script");
    for (const script of scripts) {
      try {
        const text = script.textContent || "";
        if (text.includes("_sharedData") || text.includes("__additionalData")) {
          const m = text.match(/(?:_sharedData|__additionalData)\s*[:=]\s*(\{.*?\});?\s*<\/script>/s);
          if (m) {
    ...
    // Method 2: Use fetch to get the page JSON directly.
    ...
    for (const pat of patterns) {
      ...
      if (m) {
        ...
        if (candidate && !candidate.includes("bytestart") && !candidate.includes("byteend")) {
          return candidate;
        }
      }
    }

    // Method 3: Find the video element's source in the page DOM.
    const video = document.querySelector("video");
    if (video) {
      const src = video.currentSrc || video.src || video.querySelector("source")?.src || "";
      if (src && !src.startsWith("blob:") && !src.includes("bytestart")) return src;
    }
...
// Walk through Instagram's page data structure to find the best video URL.
function extractVideoUrlFromData(data) {
  try {
    // Search for video_url in nested graphql data structures.
    const searchObj = (obj) => {
      if (!obj || typeof obj !== "object") return null;
      // Check direct video_url property.
      if (obj.video_url && typeof obj.video_url === "string" && obj.video_url.startsWith("http")) {
        return obj.video_url;
      }
      // Check video_versions array (Instagram stores multiple quality versions).
      ...
      for (const key of Object.keys(obj)) {
        const result = searchObj(obj[key]);
...
async function resolveDownloadUrl(video) {
  try {
    ...
    // Skip DASH byte-range segments — they're only a slice of bytes.
    // Scrape the full video URL from Instagram's page data.
    if (/^https?:\/\//.test(cur) && !cur.startsWith("blob:") && !cur.includes("bytestart") && !cur.includes("byteend")) return { url: cur, via: "direct" };
    const scraped = await scrapeInstagramVideoUrl();
    if (scraped) return { url: scraped, via: "scraped" };
...
/* ---------------- one menu per post (super-simple UI) ----------------
   ...
   Download, Copy caption, Copy hashtags, Copy link.
